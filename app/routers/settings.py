...
class UpdateThemeRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {"example": {"theme": "dark"}}


class UpdateAlbumBonusRequest(BaseModel):
    """Request model for album bonus updates"""

    album_bonus: float = Field(
        ..., ge=0.1, le=0.4, description="Album bonus factor (0.1 to 0.4)"
    )

    class Config:
        json_schema_extra = {"example": {"album_bonus": 0.33}}
...
class UpdateAllSettingsRequest(BaseModel):
    ...
    # General Settings
    album_bonus: Optional[float] = Field(
        None, ge=0.1, le=0.4, description="Album bonus factor"
    )
    ...
    cache_cleanup_time: Optional[str] = Field(
...
def get_settings_with_defaults(db: Session) -> UserSettings:
    ...
    if not settings:
        ...
        settings = UserSettings(
            ...
            theme="light",
            auto_migrate_artwork=os.getenv("AUTO_MIGRATE_ARTWORK", "true").lower()
            == "true",
            cache_retention_days=int(os.getenv("CACHE_RETENTION_DAYS", "365")),
            cache_max_size_mb=int(os.getenv("CACHE_MAX_SIZE_MB", "5000")),
            cache_cleanup_enabled=os.getenv("CACHE_CLEANUP_ENABLED", "true").lower()
            == "true",
            ...
            auto_cache_artwork=os.getenv("AUTO_CACHE_ARTWORK", "true").lower()
            == "true",
...
async def update_theme(request: UpdateThemeRequest, db: Session = Depends(get_db)):
    ...
    if request.theme not in ["light", "dark"]:
        raise HTTPException(
            status_code=400, detail="Theme must be either 'light' or 'dark'"
        )
...
async def update_album_bonus(
    request: UpdateAlbumBonusRequest, db: Session = Depends(get_db)
):
    """
...
async def update_all_settings(
    request: UpdateAllSettingsRequest, db: Session = Depends(get_db)
):
    """
...
async def export_database(
    service: ExportService = Depends(get_export_service), db: Session = Depends(get_db)
):
    """
...
async def import_database(
    ...
    try:
        ...
        if not is_valid:
            raise HTTPException(
                status_code=400, detail=f"Invalid backup file: {error_message}"
            )
        ...
        return ImportResponse(success=True, message=message, statistics=statistics)
...
async def update_user_settings(
    ...
    if album_bonus is not None and (album_bonus < 0.1 or album_bonus > 0.4):
        raise HTTPException(
            status_code=400, detail="Album bonus must be between 0.1 and 0.4"
        )

    if cache_cleanup_schedule is not None and cache_cleanup_schedule not in [
        "daily",
        "weekly",
        "monthly",
    ]:
    ...
    if cache_cleanup_time is not None:
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Cleanup time must be in HH:MM format (24-hour)"
            )
...
async def reset_to_defaults(db: Session = Depends(get_db)):
    ...
    if settings:
        ...
        settings.auto_migrate_artwork = (
            os.getenv("AUTO_MIGRATE_ARTWORK", "true").lower() == "true"
        )
        ...
        settings.cache_cleanup_enabled = (
            os.getenv("CACHE_CLEANUP_ENABLED", "true").lower() == "true"
        )
        ...
        settings.auto_cache_artwork = (
            os.getenv("AUTO_CACHE_ARTWORK", "true").lower() == "true"
        )
