...
class TrackRatingRequest(BaseModel):
    ...
    rating: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Track rating (0.0, 0.33, 0.67, 1.0)"
    )
...
class ComparisonRequest(BaseModel):
    ...
    album_ids: list[int] = Field(
        ...
        min_length=2,
        max_length=4,
        description="Album IDs to compare (2-4 albums)"
    )
...
class ExportRequest(BaseModel):
    ...
    include_unrated: bool = Field(
        default=False,
        description="Include unrated albums"
    )
...
class FilterParams(BaseModel):
    ...
    min_score: Optional[int] = Field(
        ...
        ge=0,
        le=100,
        description="Minimum album score"
    )
    max_score: Optional[int] = Field(
        None,
        ge=0,
        le=100,
        description="Maximum album score"
    )
    search: Optional[str] = Field(
        None,
        max_length=MAX_SEARCH_LENGTH,
        description="Search in album/artist names"
    )
