...
class ComparisonService:
    ...
    def compare_albums(
        self, album1_id: int, album2_id: int, db: Session
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if cached_result:
                logger.debug(
                    f"Returning cached comparison for albums {album1_id} vs {album2_id}"
                )
            ...
            track_matrix = self._get_track_comparison_matrix(
                album1.tracks, album2.tracks
            )

            # Calculate comparison statistics
            statistics_data = self._calculate_comparison_statistics(
                album1, album2, track_matrix
            )
            ...
            insights = self._generate_insights(
                album1, album2, statistics_data, better_tracks
            )
            ...
            logger.info(
                f"Comparison generated successfully for albums {album1_id} vs {album2_id}"
            )
            ...
            logger.error(
                f"Failed to generate comparison for albums {album1_id} vs {album2_id}: {e}"
            )
            raise TracklistException(f"Failed to generate album comparison: {str(e)}")

    def get_user_rated_albums(
    ...
    def _get_albums_for_comparison(
        self, album1_id: int, album2_id: int, db: Session
    ) -> Tuple[Album, Album]:
        """
    ...
    def _get_track_comparison_matrix(
        self, tracks1: List[Track], tracks2: List[Track]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for track_num, (track1, track2) in enumerate(aligned_pairs, 1):
            ...
            if (
                track1
                and track2
                and track1.track_rating is not None
                and track2.track_rating is not None
            ):

                diff = track1.track_rating - track2.track_rating
                comparison_row["rating_difference"] = round(diff, 2)
                comparison_row["difference_category"] = self._categorize_difference(
                    diff
                )
    ...
    def _align_tracks(
        self, tracks1: List[Track], tracks2: List[Track]
    ) -> List[Tuple[Optional[Track], Optional[Track]]]:
        """
    ...
    def _calculate_comparison_statistics(
        self, album1: Album, album2: Album, track_matrix: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        album1_ratings = [
            t.track_rating for t in album1.tracks if t.track_rating is not None
        ]
        album2_ratings = [
            t.track_rating for t in album2.tracks if t.track_rating is not None
        ]
        ...
        statistics_data = {
            "winner": {"album": winner, "score_difference": score_diff},
            "track_wins": {
        ...
        if differences:
            statistics_data["rating_differences"] = {
                ...
                "std_dev": (
                    round(statistics.stdev(differences), 3)
    ...
    def _identify_better_tracks(
        self, track_matrix: List[Dict[str, Any]], threshold: float = 0.34
        ...
        for row in track_matrix:
            if (
                row["rating_difference"] is not None
                and abs(row["rating_difference"]) >= threshold
            ):
                ...
                if row["rating_difference"] > 0:
                    track_info["track_name"] = (
                        row["album1_track"]["name"]
                        if row["album1_track"]
                        else f"Track {row['track_number']}"
                    )
                    track_info["rating"] = (
                        row["album1_track"]["rating"] if row["album1_track"] else None
                    )
                    album1_better.append(track_info)
                else:
                    track_info["track_name"] = (
                        row["album2_track"]["name"]
                        if row["album2_track"]
                        else f"Track {row['track_number']}"
                    )
                    track_info["rating"] = (
                        row["album2_track"]["rating"] if row["album2_track"] else None
                    )
    ...
    def _generate_insights(
        ...
        if track_wins["album1_wins"] > track_wins["album2_wins"]:
            highlights.append(
                f"{album1.name} has better individual tracks ({track_wins['album1_wins']} vs {track_wins['album2_wins']} track wins)"
            )
        elif track_wins["album2_wins"] > track_wins["album1_wins"]:
            highlights.append(
                f"{album2.name} has better individual tracks ({track_wins['album2_wins']} vs {track_wins['album1_wins']} track wins)"
            )
        else:
            highlights.append(
                f"Even track performance ({track_wins['album1_wins']} wins each)"
            )
        ...
        if album1_better_count > 0:
            highlights.append(
                f"{album1.name} has {album1_better_count} standout track(s)"
            )
        if album2_better_count > 0:
            highlights.append(
                f"{album2.name} has {album2_better_count} standout track(s)"
            )
    ...
    def _format_album_data(self, album: Album) -> Dict[str, Any]:
        ...
        rated_tracks = [
            t.track_rating for t in album.tracks if t.track_rating is not None
        ]
