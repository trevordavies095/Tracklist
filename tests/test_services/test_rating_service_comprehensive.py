...
from app.models import Album, Artist, ArtworkCache, Track, UserSettings
from app.rating_service import VALID_RATINGS, RatingCalculator, RatingService
...
class TestAlbumCreation:
    ...
    async def test_create_album_success(self, db_session):
        ...
        with patch.object(service, "musicbrainz_service") as mock_mb:
            ...
            with patch(
                "app.services.cover_art_service.get_cover_art_service"
            ) as mock_cover:
                mock_cover_service = Mock()
                mock_cover_service.get_cover_art_url = AsyncMock(
                    return_value="http://cover.jpg"
                )
    ...
    async def test_create_album_with_user_settings(self, db_session):
        ...
        settings = UserSettings(
            user_id=1, album_bonus=0.25, created_at=datetime.now(timezone.utc)
        )
        ...
        album = (
            db_session.query(Album).filter(Album.musicbrainz_id == "test-mbid").first()
        )
...
class TestPrivateMethods:
    ...
    def test_create_or_get_artist_new(self, db_session):
        ...
        db_artist = (
            ...
            .filter(Artist.musicbrainz_id == "artist-mbid")
            .first()
        )
