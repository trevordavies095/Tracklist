...
class ExportService:
    ...
    def export_database(self, db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            export_data = {
                "export_metadata": {
                    "version": "2.1",  # Export format version (not app version) - 2.1 adds auth fields
                ...
                export_data["albums"].append(
                    {
                        ...
                        "updated_at": (
                            album.updated_at.isoformat() if album.updated_at else None
                        ),
                        "rated_at": (
                        ...
                        "artwork_cache_date": (
                            album.artwork_cache_date.isoformat()
                            if album.artwork_cache_date
                            else None
                        ),
                    }
                )
                ...
                export_data["tracks"].append(
                    {
                        ...
                        "created_at": (
                            track.created_at.isoformat() if track.created_at else None
                        ),
                        "updated_at": (
                            track.updated_at.isoformat() if track.updated_at else None
                        ),
                    }
                )
            ...
            rated_tracks = [
                t for t in export_data["tracks"] if t["track_rating"] is not None
            ]
            ...
            logger.info(
                f"Export complete: {export_data['statistics']['total_albums']} albums, "
    ...
    def get_export_statistics(self, db: Session) -> Dict[str, Any]:
        ...
        try:
            stats = {
                ...
                "rated_tracks": db.query(Track)
                .filter(Track.track_rating.isnot(None))
                .count(),
                "settings_configured": db.query(UserSettings)
                .filter(UserSettings.user_id == 1)
                .count()
                > 0,
            }
