...
class ImportService:
    ...
    def validate_backup(self, backup_data: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            logger.info(f"Backup validation successful - Version: {metadata['version']}")
            return True, ""
    ...
    def import_database(self, db: Session, backup_data: Dict[str, Any]) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            try:
                ...
                settings = UserSettings(
                    ...
                    default_sort_order=settings_data.get('default_sort_order', 'created_desc'),
                    auto_cache_artwork=settings_data.get('auto_cache_artwork', True),
                    auto_migrate_artwork=settings_data.get('auto_migrate_artwork', False),
                    ...
                    cache_cleanup_enabled=settings_data.get('cache_cleanup_enabled', True),
                ...
                for album_data in backup_data['albums']:
                    ...
                    if album_data.get('created_at'):
                        created_at = datetime.fromisoformat(album_data['created_at'].replace('Z', '+00:00'))
                    
                    completed_at = None
                    if album_data.get('completed_at'):
                        completed_at = datetime.fromisoformat(album_data['completed_at'].replace('Z', '+00:00'))
                    ...
                    album = Album(
                        ...
                        artwork_cache_date=datetime.fromisoformat(album_data['artwork_cache_date'].replace('Z', '+00:00')) if album_data.get('artwork_cache_date') else None
                    )
                    db.add(album)
                    db.flush()
                ...
                if 'sqlite' in str(engine.url):
                    ...
                    try:
                        ...
                        result = db.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='sqlite_sequence'"))
                        if result.fetchone():
                            db.execute(text("DELETE FROM sqlite_sequence"))
                            db.execute(text(f"INSERT INTO sqlite_sequence (name, seq) VALUES ('artists', {len(backup_data['artists'])})"))
                            db.execute(text(f"INSERT INTO sqlite_sequence (name, seq) VALUES ('albums', {len(backup_data['albums'])})"))
                            db.execute(text(f"INSERT INTO sqlite_sequence (name, seq) VALUES ('tracks', {len(backup_data['tracks'])})"))
                            db.commit()
                    except Exception as e:
