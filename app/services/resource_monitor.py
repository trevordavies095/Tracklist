...
class ResourceMonitor:
    ...
    def get_memory_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "baseline_mb": (
                    round(self._baseline_memory / (1024 * 1024), 2)
                    if self._baseline_memory
                    else 0
                ),
                "growth_mb": (
                    round((mem_info.rss - self._baseline_memory) / (1024 * 1024), 2)
                    if self._baseline_memory
                    else 0
                ),
            }
    ...
    def _check_resource_health(self, report: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        health = {"overall": "healthy", "warnings": [], "errors": []}

        # Check memory growth
        if report["memory"].get("growth_mb", 0) > 100:
            health["warnings"].append(
                f"Memory growth exceeds 100MB: {report['memory']['growth_mb']}MB"
            )
            health["overall"] = "warning"

        if report["memory"].get("growth_mb", 0) > 500:
            health["errors"].append(
                f"Excessive memory growth: {report['memory']['growth_mb']}MB"
            )
        ...
        if total_connections > 50:
            health["warnings"].append(
                f"High number of connections: {total_connections}"
            )
            health["overall"] = (
                "warning" if health["overall"] == "healthy" else health["overall"]
            )
        ...
        if db_pool.get("checked_out", 0) > 10:
            health["warnings"].append(
                f"High database connections checked out: {db_pool['checked_out']}"
            )
            health["overall"] = (
                "warning" if health["overall"] == "healthy" else health["overall"]
            )
        ...
        while self._monitoring:
            try:
                ...
                if health["overall"] == "critical":
                    logger.error(
                        f"Critical resource issues detected: {health['errors']}"
                    )
