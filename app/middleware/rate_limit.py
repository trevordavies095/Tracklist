...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def _get_limiter_for_path(self, path: str) -> RateLimiter:
        ...
        for pattern, limiter in self.limiters.items():
            ...
            if "*" in pattern:
                ...
                if len(pattern_parts) == 2:
                    ...
                    if path.startswith(pattern_parts[0]) and path.endswith(
                        pattern_parts[1]
                    ):
                        return limiter
        ...
        return self.limiters.get(
            "default", self.limiters[list(self.limiters.keys())[0]]
        )


def get_rate_limiter(requests: int = 60, window_seconds: int = 60) -> RateLimiter:
    """
