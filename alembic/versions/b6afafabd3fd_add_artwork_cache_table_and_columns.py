...
def upgrade() -> None:
    # Check if artwork_cache table already exists
    conn = op.get_bind()
    inspector = sa.inspect(conn)
    tables = inspector.get_table_names()

    # Create artwork_cache table only if it doesn't exist
    if "artwork_cache" not in tables:
        op.create_table(
            "artwork_cache",
        ...
        op.create_index(
            "idx_artwork_cache_album_size",
            "artwork_cache",
            ["album_id", "size_variant"],
        )
        op.create_index("idx_artwork_cache_key", "artwork_cache", ["cache_key"])
        op.create_index(
            "idx_artwork_cache_last_accessed", "artwork_cache", ["last_accessed_at"]
        )
    ...
    if "artwork_cached" not in albums_columns:
        op.add_column(
            "albums",
            sa.Column("artwork_cached", sa.Boolean(), nullable=True, default=False),
        )

    if "artwork_cache_date" not in albums_columns:
        op.add_column(
            "albums", sa.Column("artwork_cache_date", sa.DateTime(), nullable=True)
        )

    # Clean up the schema_migrations table from our manual migration system if it exists
